...
recovery.on("restart", (event) => {
	...
	console.log(
		`[recovery] restart component=${event.component} reason=${event.reason}`,
	);
});
...
wsServer.on("error", (event: { clientId: string | null; error: Error }) => {
	console.error(
		`[ws] Error (client=${event.clientId ?? "server"}): ${event.error.message}`,
	);
});
...
inputBridge.on("reconnect", (info: { attempt: number; delay: number }) => {
	console.log(
		`[input] Reconnecting (attempt ${info.attempt}, delay ${info.delay}ms)`,
	);
});
...
async function startServer(): Promise<void> {
	try {
		...
		console.log(
			`[macstream] WebSocket server listening on port ${config.wsPort}`,
		);
	} catch (error) {
